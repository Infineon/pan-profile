...
void wiced_bt_bnep_init(void)
{
    ...
    bnep_cb.p_bnep_pool = wiced_bt_create_pool( "bnep", BNEP_MTU_SIZE + sizeof(BT_HDR) + BNEP_MINIMUM_OFFSET, BNEP_MAX_NUM_XMIT_BUF, NULL );
}

void *wiced_bt_bnep_get_pool_buf(void)
{
    wiced_bt_buffer_t *p_buf = wiced_bt_get_buffer_from_pool(bnep_cb.p_bnep_pool);
                           ...
                           uint16_t *p_handle)
{
    ...
    else
    {
        ...
        if ((cid = wiced_bt_l2cap_connect_req (BT_PSM_BNEP, p_bcb->rem_bda)) != 0)
        {
            p_bcb->l2cap_cid = cid;
                            ...
                            BOOLEAN fw_ext_present)
{
    ...
    uint8_t           *p_data;
    BT_HDR *p_buf2 = NULL;
    uint8_t offset = sizeof(BT_HDR) + BNEP_MINIMUM_OFFSET;
    ...
    if ((p_buf2 = (BT_HDR *)wiced_bt_bnep_get_pool_buf ()) == NULL)
    {
        wiced_bt_free_buffer (p_buf);
        WICED_BT_TRACE ("bnep_writeBuf() not able to get buffer");
        return (BNEP_NO_RESOURCES);
    }

    p_buf2->len = p_buf->len;
    p_buf2->offset = BNEP_MINIMUM_OFFSET;
    p_data = (uint8_t *)(p_buf2 + 1) + BNEP_MINIMUM_OFFSET;
