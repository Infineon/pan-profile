...
**                  request from the peer. PAN should call bnep_connect_resp to
                      ...
                      BOOLEAN is_role_change)
{
    ...
    */
    ...
    if (remote_uuid->len == 4)
    {
        ...
        remote_uuid->uu.uuid16 = (uint16_t)(remote_uuid->uu.uuid32 & 0x0000FFFF);
        remote_uuid->len = 2;
    }

    if (wrong_uuid)
    {
        WICED_BT_TRACE ("PAN Connection failed because of wrong remote UUID ");
        bnep_connect_resp (handle, BNEP_CONN_FAILED_SRC_UUID);
        return;
    }
    ...
    if (local_uuid->len == 4)
    {
        ...
        local_uuid->uu.uuid16 = (uint16_t)(local_uuid->uu.uuid32 & 0x0000FFFF);
        local_uuid->len = 2;
    }

    if (wrong_uuid)
    {
        WICED_BT_TRACE ("PAN Connection failed because of wrong local UUID ");
        bnep_connect_resp (handle, BNEP_CONN_FAILED_DST_UUID);
        return;
    }

    WICED_BT_TRACE ("pan_conn_ind_cb - for handle %d, current role %d, dst uuid 0x%x, src uuid 0x%x, role change %s",
    ...
    {
        WICED_BT_TRACE ("PAN Connection failed because of wrong UUID size %d", remote_uuid->len);
        bnep_connect_resp (handle, BNEP_CONN_FAILED_UUID_SIZE);
        return;
    }
#endif
    ...
    {
        WICED_BT_TRACE ("Src UUID 0x%x is not valid", remote_uuid->uu.uuid16);
        bnep_connect_resp (handle, BNEP_CONN_FAILED_SRC_UUID);
        return;
    }
    ...
    {
        WICED_BT_TRACE ("Dst UUID 0x%x is not valid", remote_uuid->uu.uuid16);
        bnep_connect_resp (handle, BNEP_CONN_FAILED_DST_UUID);
        return;
    }
    ...
    if (((!(pan_cb.role & UUID_SERVCLASS_PANU))
        ...
        && local_uuid->uu.uuid16 == UUID_SERVCLASS_NAP))
    {
        WICED_BT_TRACE ("PAN Connection failed because of unsupported destination UUID 0x%x", local_uuid->uu.uuid16);
        bnep_connect_resp (handle, BNEP_CONN_FAILED_DST_UUID);
        return;
    }
    ...
    */
    ...
    if (pcb)
    {
        if (pan_cb.num_conns > 1 && local_uuid->uu.uuid16 == UUID_SERVCLASS_PANU)
        {
            ...
            */
            WICED_BT_TRACE ("Dst UUID should be either GN or NAP only because there are other connections");
            bnep_connect_resp (handle, BNEP_CONN_FAILED_DST_UUID);
            return;
        }
        ...
        {
            WICED_BT_TRACE ("PAN Role changing New Src 0x%x Dst 0x%x",
                remote_uuid->uu.uuid16, local_uuid->uu.uuid16);
        ...
        bnep_connect_resp (handle, BNEP_SUCCESS);
        return;
    }
    else
    {
        ...
        */
        ...
        {
            WICED_BT_TRACE ("PAN already have a connection and can't be user");
            bnep_connect_resp (handle, BNEP_CONN_FAILED_DST_UUID);
            return;
        }
    }
    ...
    WICED_BT_TRACE ("New connection indication for handle %d", handle);
    pcb = pan_allocate_pcb (p_bda, handle);
    if (!pcb)
    {
        WICED_BT_TRACE ("PAN no control block for new connection");
        bnep_connect_resp (handle, BNEP_CONN_FAILED);
        return;
    }

    WICED_BT_TRACE ("PAN connection destination UUID is 0x%x", local_uuid->uu.uuid16);
    /* Set the latest active PAN role */
    pan_cb.active_role = req_role;
    pcb->src_uuid = local_uuid->uu.uuid16;
    pcb->dst_uuid = remote_uuid->uu.uuid16;
...
**
*******************************************************************************/
void pan_connect_state_cb (uint16_t handle, BD_ADDR rem_bda, tBNEP_RESULT result, BOOLEAN is_role_change)
{
    tPAN_CONN       *pcb;
    uint8_t            peer_role;
    ...
    {
        WICED_BT_TRACE ("PAN requesting for bridge");
        (*pan_cb.pan_bridge_req_cb) (pcb->rem_bda, TRUE);
    }
}
                      ...
                      BOOLEAN ext)
{
    ...
    */
    ...
    WICED_BT_TRACE ("pan_data_ind_cb - for handle %d", handle);
    pcb = pan_get_pcb_by_handle (handle);
    if (!pcb)
    {
        WICED_BT_TRACE ("PAN Data indication for wrong handle %d", handle);
        return;
    }
    ...
    if (pcb->src_uuid != UUID_SERVCLASS_PANU)
    {
        if (dst[0] & 0x01)
        {
            ...
            if (pan_cb.pan_data_buf_ind_cb)
                (*pan_cb.pan_data_buf_ind_cb) (pcb->handle, src, dst, protocol, p_data, len, ext, forward);
                            ...
                            uint8_t *p_filters)
{
    ...
    WICED_BT_TRACE ("pan_proto_filt_ind_cb - called for handle %d with ind %d, result %d, num %d",
                            ...
                            uint8_t *p_filters)
{
    ...
    WICED_BT_TRACE ("pan_mcast_filt_ind_cb - called for handle %d with ind %d, result %d, num %d",
                            handle, indication, result, num_filters);
